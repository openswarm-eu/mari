...
typedef struct {
    uint8_t length;
    uint8_t buffer[MIRA_PACKET_MAX_SIZE];
} mr_packet_t;
    ...
    if (mira_get_node_type() == MIRA_GATEWAY) {
        ...
        if (slot_type == SLOT_TYPE_SHARED_UPLINK) {
            ...
            if (len) {
                // actually pop the packet from the queue
                mr_queue_pop();
            } else if (MIRA_AUTO_UPLINK_KEEPALIVE) {
...
void mr_queue_add(uint8_t *packet, uint8_t length) {
    ...
    queue_vars.packet_queue.last = (queue_vars.packet_queue.last + 1) % MIRA_PACKET_QUEUE_SIZE;
}
...
bool mr_queue_pop(void) {
    if (queue_vars.packet_queue.current == queue_vars.packet_queue.last) {
        ...
        queue_vars.packet_queue.current = (queue_vars.packet_queue.current + 1) % MIRA_PACKET_QUEUE_SIZE;
        return true;
    }
}
