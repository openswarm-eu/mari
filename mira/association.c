...
void mr_assoc_node_register_collision_backoff(void) {
    ...
    // combine the two bytes into a 16-bit number (we need 16 bits because MIRA_BACKOFF_N_MAX > 8)
...
bool mr_assoc_node_should_leave(uint32_t asn) {
    ...
    if (assoc_vars.is_pending_disconnect != MIRA_NONE) {
        // anything other than MIRA_NONE means that the node is pending a disconnect
        return true;
    }

    bool gateway_is_lost = (asn - assoc_vars.last_received_from_gateway_asn) > mr_scheduler_get_active_schedule_slot_count() * MIRA_MAX_SLOTFRAMES_NO_RX_LEAVE;
    if (gateway_is_lost) {
        // too long since last received from the gateway, consider it lost
        assoc_vars.is_pending_disconnect = MIRA_PEER_LOST_TIMEOUT;
        return true;
    }
...
void mr_assoc_gateway_clear_old_nodes(uint64_t asn) {
    ...
    uint64_t max_asn_old = mr_scheduler_get_active_schedule_slot_count() * MIRA_MAX_SLOTFRAMES_NO_RX_LEAVE;
...
void mr_assoc_handle_beacon(uint8_t *packet, uint8_t length, uint8_t channel, uint32_t ts) {
    ...
    if (packet[1] != MIRA_PACKET_BEACON) {
        return;
    }
    ...
    if (from_my_gateway && mr_assoc_is_joined()) {
        ...
        if (!still_joined) {
            ...
            assoc_vars.is_pending_disconnect = MIRA_PEER_LOST_BLOOM;
            return;
        }
